  ...
  class HDHub4uUI {
    constructor({ container, client, perPage = 24 } = {}) {
      ...
      this.state = {
        ...
        userPref: {
          adFreePlayer: (() => { try { return localStorage.getItem('hdhub4u.adFreePlayer') !== 'false'; } catch (_) { return true; } })(),
        },
      };
...
<main class="hd-main">
  <div class="hd-grid" id="hd-grid" aria-live="polite"></div>
...
`;
    ...
    _wireShellEvents() {
      ...
      document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
          ...
          if (player && player.classList.contains('show')) this.closePlayer();
          else if (modal && modal.classList.contains('show')) this.closeModal();
        }
      });
      ...
      prefBtn.addEventListener('click', () => {
        ...
        try { localStorage.setItem('hdhub4u.adFreePlayer', String(this.state.userPref.adFreePlayer)); } catch (_) {}
    ...
    _showGridSkeletons(n = 12) {
      ...
      grid.innerHTML = Array.from({ length: n }, () =>
        `<div class="hd-card hd-card-skeleton"><div class="hd-card-poster hd-skel"></div><div class="hd-card-title hd-skel"></div><div class="hd-card-meta hd-skel"></div></div>`
      ).join('');
    }
    ...
    _renderGrid(movies) {
      ...
      grid.innerHTML = movies.map((m) => {
        const badges = m.quality && m.quality.length
        ...
        return `<article class="hd-card" data-slug="${this._esc(m.slug)}">
          <div class="hd-card-poster">${poster}${badges}</div>
    ...
    closeModal() {
      ...
      body.innerHTML = '';
      this._focusTrapActive = false;
      if (this._prevFocus) { try { this._prevFocus.focus(); } catch (_) {} this._prevFocus = null; }
    }
    ...
    openStream(playerUrl, title = 'HDHub4u Player') {
      ...
      const myId = ++this._streamRequestId;
      ...
      if (this.state.userPref.adFreePlayer) {
        ...
        this.api.resolveStream(playerUrl).then((info) => {
          if (myId !== this._streamRequestId) return;
          ...
          mount.innerHTML = `<iframe src="${this._esc(finalUrl)}" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowfullscreen sandbox="allow-scripts allow-presentation allow-forms allow-popups"></iframe>`;
        }).catch(() => {
          if (myId !== this._streamRequestId) return;
          // Fall back to sandboxed iframe.
          mount.innerHTML = `<iframe src="${this._esc(this.api.iframeStreamUrl(playerUrl, title))}" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowfullscreen sandbox="allow-scripts allow-presentation allow-forms allow-popups"></iframe>`;
        });
      } else {
        // Original sandboxed iframe of the HDHub4u player.
        srcUrl = this.api.iframeStreamUrl(playerUrl, title);
        mount.innerHTML = `<iframe src="${this._esc(srcUrl)}" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowfullscreen sandbox="allow-scripts allow-presentation allow-forms allow-popups"></iframe>`;
      }
    ...
    _renderMovieModal(m) {
        ...
        <div class="hd-movie">
          <div class="hd-movie-head">
            ...
            <div class="hd-movie-info">
              ...
              <dl class="hd-movie-meta">
                ...
                ${m.imdbUrl ? `<dt>IMDB</dt><dd><a href="${this._safeUrl(m.imdbUrl)}" target="_blank" rel="noopener">${this._esc(m.imdbId || 'View on IMDB')}</a></dd>` : ''}
              </dl>
              <div class="hd-movie-actions">
                ...
                ${m.trailer ? `<button class="hd-btn hd-btn-trailer" data-trailer="${this._safeUrl(m.trailer)}">▶ Trailer</button>` : ''}
              </div>
          ...
          ${m.screenshots && m.screenshots.length ? `
            <section class="hd-movie-section">
              <h3>Screenshots</h3>
              <div class="hd-shots">
                ${m.screenshots.slice(0, 8).map((s) => `<a href="${this._safeUrl(s)}" target="_blank"><img data-src="${this._esc(s)}" alt="Screenshot" loading="lazy"></a>`).join('')}
              </div>
            </section>` : ''}
                    ...
                    <div class="hd-dl-links">
                      ${items.map((d) => `
                        <div class="hd-dl-link">
                          <a class="hd-dl-link-main" href="${this._safeUrl(d.url)}" target="_blank" rel="noopener">
                          ...
                          <div class="hd-dl-link-ext">
                            <button class="hd-dl-ext-btn" data-action="mx"  data-url="${this._safeUrl(d.url)}" title="Open in MX Player">▶ MX</button>
                            <button class="hd-dl-ext-btn" data-action="vlc" data-url="${this._safeUrl(d.url)}" title="Open in VLC">▶ VLC</button>
                            <button class="hd-dl-ext-btn" data-action="copy" data-url="${this._safeUrl(d.url)}" title="Copy link">⧉</button>
                          </div>
                        </div>`).join('')}
                    </div>
                  </div>`).join('')}
      ...
      body.querySelectorAll('.hd-btn-trailer').forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          const player = this.container.querySelector('#hd-player');
          const mount = this.container.querySelector('#hd-player-mount');
          mount.innerHTML = `<iframe src="${this._esc(url)}" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowfullscreen sandbox="allow-scripts allow-presentation allow-popups"></iframe>`;
          player.classList.add('show');
          player.setAttribute('aria-hidden', 'false');
        });
      });
          ...
          if (action === 'mx' || action === 'vlc') {
            ...
            if (/hubstream\.art|hdstream4u\.com|hubdrive\.|hubcdn\.sbs|gadgetsweb\.xyz/i.test(url)) {
              ...
              try {
                ...
                const resolvedUrl = action === 'mx' ? (info.mxIntent || this.api.buildMxIntent(direct, m.title)) : (info.vlcUrl || this.api.buildVlcUrl(direct));
              ...
              // Direct URL — open in new tab to prevent navigate-away.
              const a = document.createElement('a');
              a.href = action === 'mx' ? this.api.buildMxIntent(url, m.title) : this.api.buildVlcUrl(url);
              a.target = '_blank';
              a.rel = 'noopener';
              a.click();
            }
