...
(function (global) {
  ...
  class HDHub4uClient {
    ...
    async resolveStream(playerUrl) {
      ...
      info._ts = Date.now();
      try {
        localStorage.setItem(LS_STREAM_PREFIX + playerUrl, JSON.stringify(info));
        // Evict stale stream cache entries if localStorage is full
      } catch (e) {
        if (e.name === 'QuotaExceededError') {
          this._evictStreamCache();
    ...
    playerUrl(directUrl, opts = {}) {
      const u = new URL('/hdhub4u/player.html', location.origin);
    ...
    /** Evict oldest stream cache entries when localStorage is full. */
    _evictStreamCache() {
      const entries = [];
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k && k.startsWith(LS_STREAM_PREFIX)) {
          try {
            const v = JSON.parse(localStorage.getItem(k));
            entries.push({ key: k, ts: v._ts || 0 });
          } catch (_) { entries.push({ key: k, ts: 0 }); }
        }
      }
      entries.sort((a, b) => a.ts - b.ts);
      const evict = Math.max(1, Math.floor(entries.length / 3));
