...
// Reverse-engineered from https://skmovies-premium.pages.dev/api/search?q=<q>&page=<n>
//
// Proxies a search query to mlsbd.co/?s=<q> and parses the result list.
...
export async function onRequest(context) {
  ...
  if (!q) return json({ ok: false, error: 'Missing ?q= param' }, 400);

  try {
    // mlsbd.co uses WordPress search
    const upstreamUrl = page > 1
      ? `${UPSTREAM}/page/${page}/?s=${encodeURIComponent(q)}`
      : `${UPSTREAM}/?s=${encodeURIComponent(q)}`;
    ...
    return json({ ok: true, q, page, items });
  } catch (e) {
