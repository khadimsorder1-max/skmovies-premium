// functions/api/south.js
// Reverse-engineered from https://skmovies-premium.pages.dev/api/south?hindi=1&page=<n>
//
// Lists south-Indian movies on mlsbd.co, optionally filtered to Hindi-dubbed only.

const UPSTREAM = 'https://mlsbd.co';
const BASE_PATH = '/category/south-indian-movies';
const HINDI_PATH = '/category/south-indian-movies/hindi-dubbed-movies';

export async function onRequest(context) {
  const { request } = context;
  const url = new URL(request.url);
  const hindi = url.searchParams.get('hindi') === '1' || url.searchParams.get('hindi') === 'true';
  const page = parseInt(url.searchParams.get('page') || '1', 10);

  try {
    const path = hindi ? HINDI_PATH : BASE_PATH;
    const upstreamUrl = `${UPSTREAM}${path}/page/${page}/`;
...
function parseList(html) {
  ...
  while ((m = postRegex.exec(html)) !== null) {
    ...
    const language = (title.match(/\b(Bengali|Hindi|English|Dual Audio)\b/i) || [])[1] || 'Hindi';
