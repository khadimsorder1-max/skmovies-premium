 ...
 */
const {
  setEnv, getEnv, setWaitUntil, eventWaitUntil,
...
async function cached(key, fetcher, ttlSec, opts = {}) {
  ...
  // Only persist non-empty results. Caching an empty/failed scrape
  // (e.g. during a transient host outage) would poison the cache
  // for the full TTL and show users a blank page.
...
/** Detect empty list/movie results we should never cache. */
function isEmptyResult(v) {
  if (!v || typeof v !== 'object') return false;
  if (Array.isArray(v.movies) && v.movies.length === 0) return true;
  if (Array.isArray(v.items) && v.items.length === 0) return true;
  if (v.movies === undefined && v.items === undefined) return false;
  return false;
}
