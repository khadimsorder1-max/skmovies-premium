export async function onRequest(context) {
  ...
  if (!q) {
    return new Response(JSON.stringify({ ok: true, page: 1, items: [], hasMore: false }), {
      headers: { 'Content-Type': 'application/json; charset=utf-8', 'Access-Control-Allow-Origin': '*' }
    });
  }

  try {
    const targetUrl = `https://fibwatch.art/search?keyword=${encodeURIComponent(q)}&page_id=${page}`;
    let html = '';
    try {
